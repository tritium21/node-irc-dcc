...
DCC.prototype.sendFile = function (to, filename, length, callback) {
    ...
    self.getPortIP((details) => {
        ...
        var timeout = setTimeout(() => {
            ...
            server.close();
            callback(new Error("Other side failed to connect in time"), null, null);
        }, self.timeout);
        ...
        server = net.createServer((con) => {
            ...
            con.setTimeout(self.timeout, () => {
                callback(new Error("Connection timed out."), null, null);
            });
...
DCC.prototype.acceptSend = function (from, host, port, filename, length, position, callback) {
    ...
    var timeout = setTimeout(() => {
        ...
        callback(new Error("Resume Send negotiation timed out."), filename, null);
    }, self.timeout);
...
DCC.prototype.sendChat = function (to, callback) {
    ...
    self.getPortIP((details) => {
        ...
        var timeout = setTimeout(() => {
            server.close();
            callback(new Error("Other side failed to connect in time"), null);
        }, self.timeout);
        server = net.createServer((con) => {
            clearTimeout(timeout);
            server.close();
...
DCC.prototype.acceptChat = function (host, port, callback) {
    ...
    con.setTimeout(self.timeout, () => {
        con.end();
        callback(new Error("Connection timed out."), null);
    });
};
