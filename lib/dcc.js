...
const myip = netUtil.getMyIP;
const getUnusedPort = netUtil.getUnusedPort;
...
function DCC(client, options) {
    var self = this;
    self.client = client;
    self.ports = options.range || 0;
    self.localAddress = options.localAddress || client.opt.localAddress || '0.0.0.0'
    self.client.addListener('ctcp-privmsg', self.ctcpListener);
}
DCC.prototype.ctcpListener = function (from, to, text, message) {
    var self = this;
